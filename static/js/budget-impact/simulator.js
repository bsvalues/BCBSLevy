...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const districtTypeApplyContainer = document.getElementById(
    "districtTypeApplyContainer",
  );
  const districtTypeFilters = document.querySelectorAll(
    ".district-type-filter",
  );
  ...
  const districtTypeApplyCheckboxes = document.querySelectorAll(
    ".district-type-apply",
  );
  ...
  // Containers
  const simulationSummaryContainer = document.getElementById(
    "simulationSummaryContainer",
  );
  const districtImpactTableBody = document.getElementById(
    "districtImpactTableBody",
  );
  const aiInsightsContainer = document.getElementById("aiInsightsContainer");
  const multiYearProjectionsContainer = document.getElementById(
    "multiYearProjectionsContainer",
  );
  const sensitivityAnalysisContainer = document.getElementById(
  ...
  if (applyToAllDistricts && districtTypeApplyContainer) {
    applyToAllDistricts.addEventListener("change", function () {
      districtTypeApplyContainer.style.display = this.checked
        ? "none"
        : "block";
    });
  }
    ...
    fetch("/budget-impact/api/simulation", {
      ...
      .then((data) => {
        ...
        if (data.success === false) {
          ...
          showErrorToast(
            "Simulation Error",
            data.error || "An error occurred during simulation",
          );
  ...
  function runAISimulation(params) {
    ...
    runSimulationBtn.disabled = true;
    runSimulationBtn.innerHTML =
      '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Processing AI Analysis...';
    ...
    fetch("/budget-impact/api/ai-simulation", {
      ...
      .then((data) => {
        ...
        if (!data.success) {
          ...
          showErrorToast(
            "AI Simulation Error",
            data.error || "Unknown error occurred with AI simulation",
          );
          ...
          if (data.baseline && data.simulation && data.impact) {
            ...
            displaySimulationResults(data);
            showInfoToast(
              "Using Standard Simulation",
              "AI simulation failed, showing standard results instead",
            );
          }
        ...
        if (data.optimization_recommendations) {
          ...
          results.ai_insights &&
          results.ai_insights.optimization_recommendations
        ) {
        ...
        showSuccessToast(
          ...
          "The AI simulation was successfully processed",
        );
      ...
      .catch((error) => {
        ...
        aiInsightsContainer.innerHTML =
          '<div class="alert alert-danger">AI simulation failed. Please try again or use standard simulation.</div>';
      });
  }
  ...
  function displayAISimulationResults(results) {
    ...
    if (simulation.district_impacts) {
      updateDistrictImpactTable(
        baseline,
        simulation,
        simulation.district_impacts,
      );
    }
  ...
  function displaySummary(baseline, simulation) {
    ...
    const levyAmountChange =
      simulation.total_levy_amount - baseline.total_levy_amount;
    const levyAmountPercent =
      (levyAmountChange / baseline.total_levy_amount) * 100;
  ...
  function updateMetrics(baseline, simulation) {
    ...
    document.getElementById("baselineTotalLevy").textContent =
      "$" + formatNumber(baseline.total_levy_amount);

    // Update simulated total levy
    document.getElementById("simulatedTotalLevy").textContent =
      "$" + formatNumber(simulation.total_levy_amount);
    ...
    // Update levy change
    const levyChangeElement = document.getElementById(
      "simulatedTotalLevyChange",
    );
    levyChangeElement.textContent =
      formatNumber(levyChangePercent) + "% change";
    levyChangeElement.className =
      "metric-change " +
      (levyChange > 0
        ? "impact-positive"
        : levyChange < 0
    ...
    if (baseline.avg_tax_per_property !== undefined) {
      document.getElementById("avgTaxPerProperty").textContent =
        "$" + formatNumber(simulation.avg_tax_per_property);

      const taxPerPropertyChange =
        simulation.avg_tax_per_property - baseline.avg_tax_per_property;
      const taxPerPropertyChangePercent =
        (taxPerPropertyChange / baseline.avg_tax_per_property) * 100;

      const taxPerPropertyChangeElement = document.getElementById(
        "avgTaxPerPropertyChange",
      );
      taxPerPropertyChangeElement.textContent =
        formatNumber(taxPerPropertyChangePercent) + "% change";
      taxPerPropertyChangeElement.className =
        "metric-change " +
        (taxPerPropertyChange > 0
          ? "impact-positive"
          : taxPerPropertyChange < 0
            ? "impact-negative"
            : "impact-neutral");
    ...
    if (baseline.avg_levy_rate !== undefined) {
      document.getElementById("avgLevyRate").textContent = formatNumber(
        simulation.avg_levy_rate,
      );
      ...
      rateChangeElement.textContent =
        formatNumber(rateChangePercent) + "% change";
      rateChangeElement.className =
        "metric-change " +
        (rateChange > 0
          ? "impact-positive"
          : rateChange < 0
  ...
  function updateDistrictImpactTable(baseline, simulation, impact) {
    ...
    if (tableHtml) {
      ...
      districtImpactTableBody.innerHTML =
        '<tr><td colspan="6" class="text-center">No district impact data available</td></tr>';
    }
  }
  ...
  function updateMultiYearProjections(baseline, projections) {
    ...
    if (multiYearChart) {
      ...
      multiYearChart.data.datasets[1].data = levyRates.map(
        (rate) => rate * 100,
      ); // Scale rates for visibility
    ...
    projections.forEach((projection) => {
      ...
      const baselineLevy = baseline.total_levy_amount;
      const levyChangePercent =
        ((levyAmount - baselineLevy) / baselineLevy) * 100;
  ...
  function displaySensitivityAnalysis(sensitivityScenarios) {
    ...
    const rateScenarios = sensitivityScenarios.filter(
      (s) => s.parameter === "rate_change_percent",
    );
    const valueScenarios = sensitivityScenarios.filter(
      (s) => s.parameter === "assessed_value_change_percent",
    );
    ...
    sensitivityScenarios.forEach((scenario) => {
      const parameter =
        scenario.parameter === "rate_change_percent"
          ? "Tax Rate Change"
  ...
  function applyOptimalScenario(rateChange, valueChange) {
    ...
    const toast = new bootstrap.Toast(
      document.getElementById("optimalAppliedToast"),
    );
    toast.show();
  }
  ...
  function displayAIInsights(insights) {
    ...
    if (
      insights.stakeholder_impacts &&
      insights.stakeholder_impacts.length > 0
    ) {
      let impactsHtml =
        '<div class="table-responsive"><table class="table table-hover">';
      impactsHtml +=
        "<thead><tr><th>Stakeholder</th><th>Impact</th><th>Scale</th></tr></thead><tbody>";
    ...
    if (
      insights.implementation_considerations &&
      insights.implementation_considerations.length > 0
    ) {
      ...
      consHtml += "</ul>";
      document.getElementById("implementationConsiderations").innerHTML =
        consHtml;
    }
    ...
    if (insights.multi_year_assessment) {
      document.getElementById("multiYearAssessmentContainer").style.display =
        "block";
      document.getElementById("multiYearAssessment").innerHTML =
        insights.multi_year_assessment;
    }
  ...
  function initializeCharts() {
    // Levy Amount Chart
    const levyAmountCtx = document
      .getElementById("levyAmountChart")
      .getContext("2d");
    ...
    // Levy Rate Chart
    const levyRateCtx = document
      .getElementById("levyRateChart")
      .getContext("2d");
    ...
    // Multi-Year Chart
    const multiYearCtx = document
      .getElementById("multiYearChart")
      .getContext("2d");
    ...
    const rateSensitivityCtx = document
      .getElementById("rateSensitivityChart")
      .getContext("2d");
    ...
    const valueSensitivityCtx = document
      .getElementById("valueSensitivityChart")
      .getContext("2d");
  ...
  function resetResults() {
    ...
    document.getElementById("executiveSummary").innerHTML =
      '<div class="loader-container"><p class="text-muted">Run AI simulation to see insights</p></div>';
    document.getElementById("keyInsights").innerHTML =
      '<div class="loader-container"><p class="text-muted">Run AI simulation to see insights</p></div>';
    document.getElementById("stakeholderImpacts").innerHTML =
      '<div class="loader-container"><p class="text-muted">Run AI simulation to see insights</p></div>';
    ...
    document.getElementById("multiYearAssessment").innerHTML =
      '<div class="loader-container"><p class="text-muted">Run AI simulation to see insights</p></div>';
    document.getElementById("optimalScenario").innerHTML =
      '<div class="loader-container"><p class="text-muted">Run AI simulation to see insights</p></div>';

    // Reset multi-year projections
    multiYearProjectionsContainer.style.display = "none";
    document.getElementById("multiYearTableBody").innerHTML =
      '<tr><td colspan="6" class="text-center">Run multi-year simulation to see results</td></tr>';
    ...
    document.getElementById("sensitivityTableBody").innerHTML =
      '<tr><td colspan="5" class="text-center">Run sensitivity analysis to see results</td></tr>';
  }
...
function applyOptimalScenario(rateChange, valueChange) {
  ...
  showSuccessToast(
    "Values Applied",
    'Optimal scenario values have been applied! Click "Run Simulation" to see results.',
  );
}
...
function enableOptimizationRecommendations(recommendations) {
  ...
  let explanation =
    "The AI has analyzed your scenario and recommends parameter adjustments to optimize your budget impact.";
  ...
  for (const [key, value] of Object.entries(recommendations)) {
    ...
    if (
      key === "tax_rate_adjustment" ||
      key === "property_value_growth" ||
      key === "explanation"
    ) {
      continue;
    }
  ...
  document
    .getElementById("applyOptimalScenario")
...
function applyOptimizationRecommendations(recommendations) {
  ...
  const optimalAppliedToast = new bootstrap.Toast(
    document.getElementById("optimalAppliedToast"),
  );
  ...
  showSuccessToast(
    "Optimal Scenario Applied",
    "The AI-recommended values have been applied to the sliders. Run the simulation again to see the optimized results.",
  );
}
