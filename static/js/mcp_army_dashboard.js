...
function setupEventListeners() {
  ...
  document
    .getElementById("executeWorkflowBtn")
    ?.addEventListener("click", executeWorkflow);
}
  ...
  fetch("/mcp-army/api/command-structure")
    ...
    .catch((error) => {
      ...
      showToast(
        "error",
        "Error",
        "Failed to load command structure. Try refreshing the page.",
      );
    });
}
  ...
  fetch("/mcp-army/api/agents")
    ...
    .then((data) => {
      ...
      document.querySelectorAll(".card-title").forEach((el) => {
        if (el.textContent.includes("Total Agents")) {
          ...
          if (
            displayElement &&
            displayElement.classList.contains("display-4")
          ) {
      ...
      const activeAgents = state.agents.filter(
        (agent) => agent.status.status === "active",
      );
    ...
    .catch((error) => {
      ...
      showToast(
        "error",
        "Error",
        "Failed to load agents. Try refreshing the page.",
      );
    });
}
...
function fetchRecentExperiences(agentId) {
  fetch(`/mcp-army/api/agents/${agentId}/experiences?limit=5`)
    ...
    .then((data) => {
      ...
      console.log(
        `Experiences loaded for ${agentId}:`,
        state.experiences[agentId].length,
      );
...
function updateRecentExperiencesDisplay() {
  ...
  if (recentExperiences.length === 0) {
    tableBody.innerHTML =
      '<tr><td colspan="3" class="text-center">No experiences recorded</td></tr>';
    return;
  }
...
function initCommunicationGraph() {
  ...
  if (!graphElement || !state.commandStructure || !state.agentRelationships)
    return;
  ...
  graphElement.innerHTML =
    '<div class="p-3 text-center">Communication Graph Visualization<br>(Placeholder for visualization library)</div>';
  ...
  if (state.commandStructure.component_leads) {
    ...
    Object.entries(state.commandStructure.component_leads).forEach(
      ([component, lead]) => {
  ...
  if (state.commandStructure.specialist_agents) {
    ...
    Object.entries(state.commandStructure.specialist_agents).forEach(
      ([domain, agents]) => {
  ...
  fetch(`/mcp-army/api/agents/${agentId}`)
    ...
    .then((agent) => {
      ...
      if (agent.capabilities && agent.capabilities.length > 0) {
        ...
        html +=
          '<div class="col-12"><p class="text-muted">No capabilities listed</p></div>';
      }
...
function fetchAgentExperiences(agentId) {
  ...
  fetch(`/mcp-army/api/agents/${agentId}/experiences?limit=5`)
    ...
    .catch((error) => {
      ...
      tableBody.innerHTML =
        '<tr><td colspan="3" class="text-center text-danger">Error loading experiences</td></tr>';
    });
}
...
function showCapabilitySelector(agentId) {
  ...
  if (!agent || !agent.capabilities || agent.capabilities.length === 0) {
    showToast(
      "warning",
      "No Capabilities",
      `Agent ${agentId} has no registered capabilities.`,
    );
    return;
  }
...
function executeCapability(agentId, capability) {
  ...
  showToast(
    ...
    `Requesting ${formatCapabilityName(capability)} from ${agentId}...`,
  );
  ...
  document
    .getElementById("executeCapabilityBtn")
    ?.addEventListener("click", function () {
      const paramsText =
        document.getElementById("capabilityParams")?.value || "{}";
      let params;
      ...
      try {
        ...
        showToast(
          "error",
          "Invalid JSON",
          "Please provide a valid JSON object for parameters.",
        );
        return;
      }
      ...
      fetch(`/mcp-army/api/agents/${agentId}/capabilities/${capability}`, {
        ...
        .then((result) => {
          ...
          showToast(
            "success",
            "Capability Executed",
            `Successfully executed ${formatCapabilityName(capability)}.`,
          );
          ...
          addNotification(
            "capability",
            `Agent ${agentId} executed ${formatCapabilityName(capability)}`,
          );
        })
        .catch((error) => {
          console.error(
            `Error executing capability ${capability} for agent ${agentId}:`,
            error,
          );
          showToast(
            "error",
            "Execution Failed",
            `Failed to execute ${formatCapabilityName(capability)}.`,
          );
        });
...
function requestHelp(agentId) {
  ...
  if (helpers.length === 0) {
    showToast(
      "warning",
      "No Helpers",
      "No other agents are available to help.",
    );
    return;
  }
...
function submitHelpRequest(agentId, helperId) {
  ...
  showToast(
    "info",
    "Requesting Help",
    `Requesting assistance from ${helperId} for ${agentId}...`,
  );
  ...
  fetch(`/mcp-army/api/agents/${helperId}/assistance/${agentId}`, {
    ...
    .then((result) => {
      ...
      showToast(
        "success",
        "Help Requested",
        `Successfully requested help from ${helperId}.`,
      );

      // Increment help request count
      state.helpRequests++;
      document.getElementById("help-request-count").textContent =
        state.helpRequests;

      // Add a notification to the activity feed
      addNotification(
        "assistance",
        `Agent ${helperId} is assisting ${agentId}`,
      );
    })
    .catch((error) => {
      console.error(
        `Error requesting help from ${helperId} for ${agentId}:`,
        error,
      );
      showToast(
        "error",
        "Request Failed",
        `Failed to request help from ${helperId}.`,
      );
    });
}
  ...
  fetch("/mcp-army/api/training/start", {
    ...
    .then((result) => {
      ...
      showToast(
        "success",
        "Training Started",
        "Successfully started training cycle.",
      );
...
function openWorkflowModal(workflowName) {
  ...
  document.getElementById("workflowModalLabel").textContent =
    `Execute ${formatWorkflowName(workflowName)} Workflow`;
...
function executeWorkflow() {
  ...
  const additionalParamsText =
    document.getElementById("additionalParams").value;

  console.log(
    `Executing workflow ${workflowName} for district ${taxDistrictId} in year ${year}`,
  );
  ...
  showToast(
    "info",
    "Executing Workflow",
    `Executing ${formatWorkflowName(workflowName)} workflow...`,
  );
  ...
  fetch("/mcp-army/api/workflows/collaborative", {
    ...
    .then((result) => {
      ...
      showToast(
        "success",
        "Workflow Completed",
        `Successfully executed ${formatWorkflowName(workflowName)} workflow.`,
      );
      ...
      addNotification(
        "workflow",
        `Executed ${formatWorkflowName(workflowName)} workflow`,
      );
    })
    .catch((error) => {
      ...
      showToast(
        "error",
        "Execution Failed",
        `Failed to execute ${formatWorkflowName(workflowName)} workflow.`,
      );
    });
}
