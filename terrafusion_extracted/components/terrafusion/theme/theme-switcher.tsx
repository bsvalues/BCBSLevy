...
export function ThemeSwitcher({
  ...
  const { theme, mode, toggleTheme, toggleMode, setTheme, setMode } =
    useTheme();
    ...
    <div className={`relative ${className}`}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-48 bg-[#001529] border border-[#00e5ff]/20 rounded-md shadow-lg z-50">
          <div className="p-2">
            <h3 className="text-xs font-medium text-[#00e5ff]/50 px-3 py-1">
              Mode
            </h3>
            <Button
              variant="ghost"
              className={`w-full justify-start text-left px-3 py-1.5 text-sm ${
                mode === "light"
                  ...
                  : "text-[#00e5ff]/70 hover:bg-[#00e5ff]/10"
              }`}
              onClick={() => {
                setMode("light");
                setIsOpen(false);
